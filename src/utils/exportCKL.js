...
export function exportCKL(stig, hostname = "", ip = "", mac = "", fqdn = "") {
  ...
  lines.push("<!--DISA STIG Viewer :: Web STIG Tools Export-->");
  ...
  for (const rule of stig.rules) {
    ...
    lines.push(
      `        <STATUS>${STATUS_CKL_MAP[rule.status] ?? "Not_Reviewed"}</STATUS>`,
    );
    lines.push(
      `        <FINDING_DETAILS>${escXml(rule.findingDetails)}</FINDING_DETAILS>`,
    );
